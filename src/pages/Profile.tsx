...
import { ArrowLeft, Check, Eye, EyeOff, KeyRound, UserRound } from 'lucide-react';
...
const Profile = () => {
  ...
  return <div className="flex flex-col min-h-screen bg-gradient-to-b from-background to-gray-50">
      ...
      ease: "easeOut"
        ...
        <Button variant="ghost" onClick={handleBackNavigation} type="button" className="p-5 rounded-full hover:bg-white/20 text-white">
      ...
      <div className="flex-1 container max-w-md mx-auto p-5 pb-8">
        ...
        duration: 0.4
          ...
          <Avatar className="h-28 w-28 border-4 border-white shadow-lg mb-5">
            <AvatarImage src={`https://api.dicebear.com/7.x/initials/svg?seed=${employeeData.name}`} alt={employeeData.name} className="bg-tanseeq/10" />
        ...
        delay: 0.4
          ...
          {!showChangePassword ? <Button variant="default" className="w-full py-2.5 rounded-lg shadow-sm flex items-center justify-center gap-2.5 text-base" onClick={() => setShowChangePassword(true)}>
              <KeyRound className="h-5 w-5" />
              Change Password
            </Button> : <Card className="overflow-hidden bg-white shadow-md rounded-xl border-gray-100 mt-4">
